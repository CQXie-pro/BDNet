import torch
import torch.nn as nn
import torch.nn.functional as F


class Model(nn.Module):
    """
    Paper link: https://arxiv.org/pdf/2205.13504.pdf
    """

    def __init__(self, configs, individual=True):
        """
        individual: Bool, whether shared model among different variates.
        """
        super(Model, self).__init__()
        self.task_name = configs.task_name
        self.seq_len = configs.seq_len
        self.pred_len = configs.pred_len

        self.individual = individual
        self.channels = configs.enc_in
        self.dropout = nn.Dropout(configs.dropout)
        self.base_T = configs.base_T
        self.H_order = configs.H_order

        self.dominance_freq = int(configs.seq_len // self.base_T + 1) * self.H_order + 10
        self.length_ratio = (self.seq_len + self.pred_len) / self.seq_len

        # complex layer for frequency upcampling]
        if self.individual:
            self.freq_upsampler = nn.ModuleList()
            for i in range(self.channels):
                self.freq_upsampler.append(
                    nn.Linear(self.dominance_freq, int(self.dominance_freq * self.length_ratio)).to(torch.cfloat))
        else:
            self.freq_upsampler = nn.Linear(self.dominance_freq,
                                            int(self.dominance_freq * self.length_ratio)).to(torch.cfloat)

    def encoder(self, x):
        # x: [B, L, D]
        # RIN
        x_mean = torch.mean(x, dim=1, keepdim=True)
        x = x - x_mean
        x_var = torch.var(x, dim=1, keepdim=True) + 1e-5
        x = x / torch.sqrt(x_var)
        # print("x shape: ", x.shape)  # [32, 96, 862]

        # rFFT
        low_specx = torch.fft.rfft(x, dim=1)
        # print("low_specx shape: ", low_specx.shape)  # [32, 49, 862]

        # LPF
        low_specx[:, self.dominance_freq:] = 0
        low_specx = low_specx[:, 0:self.dominance_freq, :]
        # print("low_specx shape: ", low_specx.shape)  # [32, 20, 862]

        # Linear Layer
        if self.individual:  # individual layer
            low_specxy_ = torch.zeros(
                [low_specx.size(0), int(self.dominance_freq * self.length_ratio), low_specx.size(2)],
                dtype=low_specx.dtype).to(low_specx.device)
            for i in range(self.channels):
                low_specxy_[:, :, i] = self.freq_upsampler[i](low_specx[:, :, i].permute(0, 1)).permute(0, 1)
        else:  # share layer
            # print(low_specx.shape)
            low_specxy_ = self.freq_upsampler(low_specx.permute(0, 2, 1)).permute(0, 2, 1)

        # zero padding
        low_specxy = torch.zeros(
            [low_specxy_.size(0), int((self.seq_len + self.pred_len) / 2 + 1), low_specxy_.size(2)],
            dtype=low_specxy_.dtype).to(low_specxy_.device)
        low_specxy[:, 0:low_specxy_.size(1), :] = low_specxy_

        # irFFT
        low_xy = torch.fft.irfft(low_specxy, dim=1)
        low_xy = low_xy * self.length_ratio  # energy compemsation for the length change

        # iRIN
        xy = low_xy * torch.sqrt(x_var) + x_mean

        return xy

    def forecast(self, x_enc):
        # Encoder
        return self.encoder(x_enc)

    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            dec_out = self.forecast(x_enc)
            return dec_out[:, -self.pred_len:, :]  # [B, L, D]
        return None
